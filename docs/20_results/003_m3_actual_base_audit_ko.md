...
Status: holdout_base_audit_complete

## 1. 목적

Qwen3-VL-4B base model이 MVP holdout task를 너무 쉽게 풀거나 너무 못 풀지 않는지 actual
evaluation으로 확인한다. 이 결과는 LoRA certification이 아니라 M5/M6 전에 task 난이도를
판정하기 위한 base difficulty evidence다.
...
adapter: none
manifest: data/mvp/holdout.jsonl
samples: 128 total, 64 per taxonomy
...
result_json: results/m3/qwen3_vl_4b_base_holdout.json
```
...
```text
taxonomy | count | correct | score | M3 difficulty status
document_field_bind | 64 | 61 | 0.953125 | too_easy
chart_table_cell | 64 | 47 | 0.734375 | usable
overall | 128 | 108 | 0.843750 | pass_but_high
```
...
`document_field_bind`는 대부분 맞혔고, 실패 3개는 부분 답변 또는 과잉 답변이었다.
...
`chart_table_cell`은 table/chart value lookup에서 17개를 틀렸다. 대표 실패는 인접 숫자,
비율 계산, row/column binding 혼동이다.
...
M3 holdout base audit는 actual run으로 완료되었다.
...
- `chart_table_cell`은 base score가 0.734375라 M5/M6의 adapter-sensitive task로 먼저 쓰기 좋다.
